...
export function OwnerDashboard() {
  ...
  const todayBookings = OWNER_COURTS.flatMap((c) =>
    c.subCourts.flatMap((sc) =>
      sc.todayBookings.filter(
        (b) => b.status === "banked" || b.status === "booked",
      ),
    ),
  );
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          label="Đánh giá TB"
          value={`${avgRating} ⭐`}
          sub={`Từ ${OWNER_REVIEWS.length} đánh giá`}
          icon={<Star size={18} style={{ color: "#EF4444" }} />}
          color="#EF4444"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        >
          ...
          >
            Tỷ lệ đặt sân theo giờ
          </p>
          <div
            className="flex flex-col gap-2 overflow-y-auto"
            style={{ maxHeight: "200px" }}
          >
            {PEAK_HOURS.sort((a, b) => b.bookingRate - a.bookingRate)
              .slice(0, 8)
              .map((h) => (
                      ...
                      style={{
                        ...
                        background:
                          h.bookingRate >= 85
                            ...
                            : h.bookingRate >= 65
                              ? "linear-gradient(90deg,#F59E0B,#FFB300)"
                              : "linear-gradient(90deg,#00C896,#00897B)",
                      }}
                    />
                  </div>
                  <span
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h3
              style={{ fontWeight: 700, color: "#1a1a2e", fontSize: "0.95rem" }}
            >
          ...
          <div className="flex flex-col gap-3">
            {OWNER_COURTS.map((court) => {
              const allSlots = court.subCourts.flatMap(
                (sc) => sc.todayBookings,
              );
              const booked = allSlots.filter((s) =>
                ["booked", "banked", "complete"].includes(s.status),
              ).length;
              const total = allSlots.filter(
                (s) => s.status !== "blocked",
              ).length;
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p style={{ fontSize: "0.72rem", color: "#9ca3af" }}>
                      {court.subCourts.length} sân con • {court.district}
                    </p>
                  </div>
            ...
            {OWNER_REVIEWS.slice(0, 4).map((r) => (
              <div key={r.id} className="flex gap-3">
                <div
                  ...
                  style={{
                    background: `hsl(${r.avatar.charCodeAt(0) * 40}, 60%, 50%)`,
                  }}
                >
                  {r.avatar}
                </div>
                  ...
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <div className="flex gap-0.5">
                      {Array.from({ length: r.rating }).map((_, i) => (
                        <span
                          key={i}
                          style={{ fontSize: "0.6rem", color: "#FFB300" }}
                        >
                          ★
                        </span>
                      ))}
      ...
      {/* Alert */}
      <div
        className="mt-4 flex items-start gap-3 p-4 rounded-2xl"
      ...
      >
        ...
        />
        <div>
          <p style={{ fontWeight: 700, fontSize: "0.85rem", color: "#92400E" }}>
            Sân A4 đang tắt hoạt động
          </p>
          <p style={{ fontSize: "0.78rem", color: "#78350F" }}>
            Sân A4 hiện đang ở trạng thái inactive. Bật lại để nhận thêm đơn đặt
            sân.
          </p>
        </div>
        <Button
          onClick={() => navigate("/owner/courts")}
          className="ml-auto flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-bold h-auto"
