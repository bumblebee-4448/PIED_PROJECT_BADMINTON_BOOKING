...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="bg-white rounded-xl px-3 py-2 shadow-lg" style={{ border: "1px solid #e5e7eb" }}>
      <p style={{ fontSize: "0.72rem", fontWeight: 700, color: "#1a1a2e", marginBottom: "4px" }}>{label}</p>
      {payload.map((p: any) => (
...
export function VisitorChart({ data, visitorsToday, visitorsThisWeek }: VisitorChartProps) {
  return (
    <div className="lg:col-span-2 bg-white rounded-2xl p-5" style={{ border: "1px solid rgba(0,0,0,0.06)" }}>
      <div className="mb-4">
        <h3 style={{ fontWeight: 700, color: "#1a1a2e", fontSize: "0.95rem" }}>Lượt truy cập hệ thống</h3>
        <div className="flex gap-4 mt-2">
          <div>
            <p style={{ fontWeight: 800, fontSize: "1.3rem", color: "#6366F1" }}>{visitorsToday.toLocaleString()}</p>
            <p style={{ fontSize: "0.7rem", color: "#9ca3af" }}>Hôm nay</p>
          </div>
          <div>
            <p style={{ fontWeight: 800, fontSize: "1.3rem", color: "#00897B" }}>{visitorsThisWeek.toLocaleString()}</p>
